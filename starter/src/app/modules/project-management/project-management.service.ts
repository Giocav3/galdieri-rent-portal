...
import {
    ...
    Label,
    List,
} from 'app/modules/project-management/project-management.models';
import {
    BehaviorSubject,
    Observable,
    map,
...
createCard(card: Card): Observable<Card> {
    return this._httpClient
        ...
        .pipe(
            ...
            tap((newCard) => {
                ...
                if (!board) {
                    console.warn('⚠️ Board non presente, impossibile aggiungere la card');
                    return;
                }

                // Trova la lista e aggiungi la nuova card
                const listIndex = board.lists.findIndex(list => list.id === newCard.listId);

                if (listIndex === -1) {
                    console.warn('⚠️ Lista non trovata per la nuova card:', newCard);
                    return;
                }
    ...
    addProject(project: Board): void {
        const current = this._boards.value;
        this._boards.next([project, ...current]);
    }
